...
namespace Snowball.Images
{
    public static class PNG
    {
        ...
        public static ImageData Load(Stream stream)
        {
            ...
            {
                throw new SnowballException("PNG header incorrect.");
            }
            ...
            if (idat == null)
                throw new SnowballException("No IDAT chunk found.");
                    ...
                    for (int i = 0; i < height; i++)
                    {
                        ...
                        if (scanlineFilterAlgorithm != 0)
                        {
                            for (int x = 0; x < scanline.Length; x++)
                            {
                                ...
                                {
                                    case 1: // Sub
                                        throw new SnowballException("Sub filter algorithm in PNG not implemented.");
                                    ...
                                    case 3: // Average
                                        throw new SnowballException("Average filter algorithm in PNG not implemented.");

                                    case 4: // Paeth
                                        throw new SnowballException("Paeth filter algorithm in PNG not implemented.");
                                }
                            }
                        }
            ...
            for (int i = 0; i < pixels.Length; i++)
            {
                switch (bytesPerPixel)
                {
                    ...
                    default:
                        throw new SnowballException($"PNG loading not implemented for images with {bytesPerPixel} bytes per pixel.");
                }
            }
